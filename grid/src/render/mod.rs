...
pub fn render_app_builder(app: &mut App) {
    load_internal_asset!(app, GRID_SHADER_HANDLE, "shader.wgsl", Shader::from_wgsl);
...
impl GridDisplaySettingsUniform {
    fn from_settings(settings: &InfiniteGridOptions) -> Self {
        Self {
...
fn extract_per_camera_settings(
    mut commands: Commands,
    cameras: Extract<Query<(Entity, &InfiniteGridOptions), With<Camera>>>,
) {
...
fn prepare_infinite_grids(
    ...
    cameras: Query<(Entity, &InfiniteGridOptions), With<ExtractedView>>,
...
fn queue_infinite_grids(
    ...
    for (view_entity, entities, view) in views.iter_mut() {
        ...
        for &entity in entities.iter::<With<InfiniteGridOptions>>() {
            if !infinite_grids
                .get(entity)
